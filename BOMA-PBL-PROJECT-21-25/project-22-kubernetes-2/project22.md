...
NOTE: Please not that we are not using the clusters built in [project 21](../project-21-kubernetes-1) to deploy our applications. The following [link](https://eksctl.io/) is used to setup new cluster that will be use in this project

[EKS-manifest](Configuration-codes/boma-eks-cluster.yaml)

To use EKS ensure the followings are installed on your local machine
...
![Alt text](Images/kubernetes-hybrid-architecture.png)
...
nginx-pod   1/1     Running   0          57m   172.50.197.236   ip-172-50-197-215.us-west-2.compute.internal   <none>           <none>
```
...
![nginx-pod-output](Images/nginx-pod-output.png)

![nginx-running pod](Images/nginx-pod-running.png)

![nginx-web-page](Images/nginx-page.png)
...
There are other patterns, such as: [init container, adapter container, ambassador container](https://medium.com/bb-tutorials-and-thoughts/kubernetes-learn-ambassador-container-pattern-bc2e1331bd3a). These are more advanced topics that you can study on your own, let us continue with the other objects.
...
We need an image that already has curl software installed. You can check it out [here](https://hub.docker.com/r/dareyregistry/curl)
...
![portforwarding](Images/kubernetes-portforwarding.png)
...
![portforwarding1](Images/portfowarding-result.png)

![portforded-page](Images/portforwarded-webpage.png)
...
In general, there are 2 approaches of [Kubernetes](https://kubernetes.io/docs/concepts/overview/working-with-objects/object-management/) Object Management: imperative and declarative.
...
```markdown
NAME            TYPE           CLUSTER-IP      EXTERNAL-IP                                                                  PORT(S)        AGE
nginx-service   LoadBalancer   10.100.71.130   aab159950f39e43d39195e23c77417f8-1167953448.us-west-1.elb.amazonaws.com   80:31388/TCP   5d18h
```
An ELB resource will be created in your AWS console.

![LB1](Images/LB-cli.png)

![LB2](Images/LB-created.png)
...
![LB-web-page](Images/LB-webpage.png)
...
Previously, you have seen the improvements from **ReplicationControllers (RC)**, to ```markdown
ReplicaSets (RS)
```. In this section you will see another K8s object which is highly recommended over Replication objects (RC and RS).
...
nginx-deployment-56466d4948-xctfh   1/1     
Running   0          6s
```
...
![k8s-workload](Images/kubernetes-workload.png)
...
4. Update the content of the file and add the code below ```markdown
...
![updated-deployment](Images/deployment-updated.png)
...
![nginx-default](Images/LB-webpage.png)

Storage is a critical part of running containers, and Kubernetes offers some powerful primitives for managing it. ```markdown
Dynamic volume provisioning
```, a feature unique to Kubernetes, which allows storage volumes to be created on-demand. Without dynamic provisioning, DevOps engineers must manually make calls to the cloud or storage provider to create new storage volumes, and then create 
**PersistentVolume**
 objects to represent them in Kubernetes. The dynamic provisioning feature eliminates the need for DevOps to pre-provision storage. Instead, it automatically provisions storage when it is requested by users.
