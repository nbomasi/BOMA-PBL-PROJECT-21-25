...
I created the EKS Cluster using the following k8s manifest and links: 

[eks-cluster-manifest](../project-22-kubernetes-2/Configuration-files/boma-eks-cluster.yaml)

[link](https://eksctl.io/)

The following link will give you the information needed to get the necessary IAM permision on eks cluster that will allow you to create and attach volume to K8s nodes.

[eks-cluster](https://medium.com/@kaitylegrande/its-easy-deploy-highly-available-wordpress-app-on-eks-cluster-with-prometheus-using-helm-e7a8150321ef)

[iam-pv-permission](https://github.com/onyeka-hub/ebs-volume-mount-in-amazon-eks)

NOTE: Even though we are going through through the process of creating persistent volume for our Nodes, note that PVs are created by default when we create EKS clusters.
...
* **Volume** of 10Gi was manually created on AWS console and the **volumeID** was noted to be used in attaching the volume to the nodes.
...
```markdown
kubectl get po nginx-deployment-6fdcffd8fc-thcup -o wide
```
...
![node-info-for-AZ](Images/node-info.png)

The above image helped us to know the AZ to create our volume, since the volume must be created in the same AZ as the node, in the above the AZ in us-east-1a.

The **Volume ID** will now be use in the following **manifest**
...
```
![pod-running-attached-pv](Images/manual-vol.png)
...
![munual-pv](Images/manual-mounted-pv.png)
...
Please note that Gi is almost the same as G, but not exactly the same.
...
more information about the different types of persistent volumes [here](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#access-modes)
...
to demonstrate this, we will use the HTML file that came with Nginx. This file can be found in **/usr/share/nginx/html/index.htm**l  directory.
...
```markdown
kubectl exec -it nginx-deployment-79d8c764bc-j6sum -- bash
```
  
  ```markdown
  cat /usr/share/nginx/html/index.html
  ``` 
![html-content](Images/output-html.png)
...
![configmap-page](Images/configmap-webpage.png)

You can restart the deployment for any reason, simply use the command
...
And every configuration file will be intact.
